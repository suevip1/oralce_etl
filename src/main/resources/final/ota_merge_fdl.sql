...
FROM (
    WITH t_0 AS (
        SELECT t.*,
            (case when t1.istDates IS NULL THEN NULL ELSE (SELECT NDATE
            FROM (
                SELECT NDATE, diffTime, rownum AS rn
                FROM (
                    SELECT ROUND(TO_NUMBER(td.D - t.CREATE_TIME) * 24 * 60 * 60) AS diffTime, td.D AS NDATE FROM table(istDates) td
                    WHERE ROUND(TO_NUMBER(td.D - t.CREATE_TIME) * 24 * 60 * 60) >= 0
                    ORDER BY diffTime
                )
            )
            WHERE rn = 1) END) AS nearTime1,
            (case when t1.istDates IS NULL THEN NULL ELSE (SELECT NDATE
            FROM (
                SELECT NDATE, diffTime, rownum AS rn
                FROM (
                    SELECT ROUND(TO_NUMBER(t.CREATE_TIME - td.D) * 24 * 60 * 60) AS diffTime, td.D AS NDATE FROM table(istDates) td
                    WHERE ROUND(TO_NUMBER(t.CREATE_TIME - td.D) * 24 * 60 * 60) >= 0
                    ORDER BY diffTime
                )
            )
            WHERE rn = 1) END) AS nearTime2
        FROM %2$s t
        LEFT JOIN (
    ...
    t_1 AS (
        SELECT t.*, (CASE WHEN nearTime1 IS NOT NULL THEN nearTime1 ELSE nearTime2 END) NEARTIME
        FROM t_0 t
    ),
    t_2 AS (
    ...
    FROM t_2 t
    LEFT JOIN %3$s t1
    ON t.FLIGHT_DATE = t1.FLIGHT_DATE AND t.DEP = t1.DEP AND t.ARR = t1.ARR AND t.FLIGHT_NO = t1.fltno AND t.NEARTIME = t1.INSERT_DATE
) t
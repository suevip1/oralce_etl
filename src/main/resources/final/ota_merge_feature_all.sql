...
FROM (
    ...
    t_3 AS (
        ...
        AND ROUND(TO_NUMBER(TO_DATE(flight_date) - TO_DATE(INSERT_DATE))) <= %9$s
        AND %10$s
    ),
    t_4 AS (
        SELECT t.*
    ...
    t_8 AS (
        SELECT FLIGHT_DATE, AIR_CODE, FLIGHT_NO, DEP_TIME, DEP, ARR, CREATE_TIME, LABEL, TOP_1, TOP_2, TOP_3, TOP_4, TOP_5,
            ...
            JF_FLIGHT_NO, JF_FEA_HR, JF_COM_MARKET_PER, JF_CAPACITY_P,
            JF_OD_MARKET_PER, JF_KZL, JF_FEA_FLIGHT_NO, JF_AIRCODE_CLASS, JF_PRICE, PRICE_GAP, JF_SINGLE_LEG_TIME,
            JF_AC_SINGLE_LEG_TIME, JF_DEP, JF_ARR, JF_FEA_HR_CLASS,
            (CASE WHEN jgcha2 > -1 and jgcha2 <= 3 THEN 1 WHEN jgcha2 > 3 THEN 2 ELSE 0 END) as JF2_TYPE,
            JF2_FLIGHT_NO, JF2_FEA_HR, JF2_CAPACITY_P,
            JF2_AIRCODE_CLASS, JF2_KZL, JF2_PRICE, JF2_SINGLE_LEG_TIME, JF2_AC_SINGLE_LEG_TIME, JF2_DEP, JF2_ARR,
            JF2_FEA_HR_CLASS, LOWEST_FLIGHT_NO, LOWEST_DEP_TIME, LOWEST_PRICE, FPRICE, CPRICE, JF_FPRICE, JF_CPRICE,
            JF2_FPRICE, JF2_CPRICE, JF_ID, JF2_ID
        FROM t_7
    ),
    t_9 AS (
        SELECT t.*, (CASE WHEN t1.flt_no IS NOT NULL THEN bkg ELSE -2 END) as pnr_bkg,
               (CASE WHEN t1.flt_no IS NOT NULL THEN min_price ELSE -2 END) as min_price,
               (CASE WHEN t1.flt_no IS NOT NULL THEN max_price ELSE -2 END) as max_price
        FROM t_8 t
        LEFT JOIN %6$s t1
        ON t.flight_date= t1.flight_date AND t.dep = t1.dep AND t.arr = t1.arr AND t.flight_no = t1.flt_no AND t.ex_dif = t1.ex_dif
    ),
    t_10 AS (
        SELECT *
        FROM (
    ...
    t_x AS (
        SELECT t.*,
            (CASE WHEN pnr_bkg = -2 THEN label WHEN (pnr_bkg = -1 and t.label > max_price and max_price > 0) THEN max_price
                WHEN (pnr_bkg = -1 and t.label > max_price and max_price <= 0) THEN label
                WHEN (pnr_bkg = -1 and t.label <= max_price and min_price > 0) THEN min_price
                WHEN (pnr_bkg = -1 and t.label <= max_price and min_price <= 0) THEN label
                WHEN (pnr_bkg = 0 and t.label > min_price and min_price > 0) THEN min_price
                WHEN (pnr_bkg = 0 and t.label > min_price and min_price <= 0) THEN label
                WHEN (pnr_bkg = 0 and t.label <= min_price) THEN label
                WHEN (pnr_bkg > 0 and t.label <= max_price) THEN label
                WHEN (pnr_bkg > 0 and t.label > max_price and max_price > 0) THEN max_price
                  WHEN (pnr_bkg > 0 and t.label > max_price and max_price <= 0) THEN label END) as label_pnr_ota
        FROM t_10 t
    )
    SELECT FLIGHT_DATE, AIR_CODE, FLIGHT_NO, DEP_TIME, DEP, ARR, CREATE_TIME, LABEL, TOP_1, TOP_2, TOP_3, TOP_4, TOP_5, TOP_6,
        FEA_AIR_CODE, FEA_FLIGHT_NO, FEA_DEP, FEA_ARR, EX_DIF, FEA_HR, FEA_HR_CLASS, AIRCODE_CLASS, FEA_NEAREST_TIME_1,
        FEA_NEAREST_TIME_2, FEA_DELT_DEPTIME, TOP_MEAN, TOP_MAX, TOP_MIN, TOP_STD, FLIGHTS_SELF, FLIGHTS_TOTAL, POWER,
        FEA_MONTH, FEA_DAY, FEA_WEEK, FLIGHT_TYPE_CODE, SKEY, H_EX_DIF, IST_HOUR, COMP, EQT, FLTNO, DEPTIME, CAP, MAX,
        BKD, REST, KZL, EQT_CODE, INSERT_DATE, DISTANCE, STD_PRICE, HDLX, OD, HX, ZGLSR, SUM_INCOME, FLY_TIME, CAP_P,
        CAP_SUM, CAPACITY, COM_MARKET_PER, CAP_C, CAP_COUNT, AREA_MARKET_PER, CAPACITY_P, OD_MARKET_PER, HOLIDAY, YOY_DATE,
        YOY_BKD, YOY_REST, SINGLE_LEG_TIME, AC_SINGLE_LEG_TIME, LAST_3_FLY_NUM, LAST_3_FLY_COUNT, LAST_3_FLY_RATE,
        LAST_2_FLY_NUM, LAST_2_FLY_COUNT, LAST_2_FLY_RATE, LAST_1_FLY_NUM, LAST_1_FLY_COUNT, LAST_1_FLY_RATE, FLY_NUM,
        FLY_COUNT, FLY_RATE, AFTER_1_FLY_NUM, AFTER_1_FLY_COUNT, AFTER_1_FLY_RATE, AFTER_2_FLY_NUM, AFTER_2_FLY_COUNT,
        AFTER_2_FLY_RATE, AFTER_3_FLY_NUM, AFTER_3_FLY_COUNT, AFTER_3_FLY_RATE, KZL_P, KZL_HIS_1, KZL_HIS_2, KZL_HIS_3,
        KZL_HIS_4, KZL_HIS_5, KZL_HIS_6, KZL_HIS_7, JF_TYPE, JF_FLIGHT_NO, JF_FEA_HR, JF_COM_MARKET_PER, JF_CAPACITY_P,
        JF_OD_MARKET_PER, JF_KZL, JF_FEA_FLIGHT_NO, JF_AIRCODE_CLASS, JF_PRICE, PRICE_GAP, JF_SINGLE_LEG_TIME,
        JF_AC_SINGLE_LEG_TIME, JF_DEP, JF_ARR, JF_FEA_HR_CLASS, JF2_TYPE, JF2_FLIGHT_NO, JF2_FEA_HR, JF2_CAPACITY_P,
        JF2_AIRCODE_CLASS, JF2_KZL, JF2_PRICE, JF2_SINGLE_LEG_TIME, JF2_AC_SINGLE_LEG_TIME, JF2_DEP, JF2_ARR,
        JF2_FEA_HR_CLASS, LOWEST_FLIGHT_NO, LOWEST_DEP_TIME, LOWEST_PRICE, FPRICE, CPRICE, JF_FPRICE, JF_CPRICE, JF2_FPRICE,
        JF2_CPRICE, PNR_BKG, MIN_PRICE, MAX_PRICE, LABEL_PNR_OTA,
        (CASE WHEN jf_price >= 0 THEN label_pnr_ota - jf_price ELSE -1 END) as LABEL_PNR_OTA_JF_GAP, JF_ID, JF2_ID
    FROM t_x t
) t
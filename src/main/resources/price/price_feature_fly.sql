...
FROM (
    ...
    t_9 AS (
        SELECT t.*,
            (CASE WHEN t1.CREATE_TIMES IS NULL THEN NULL ELSE (SELECT cDate
            FROM (
                SELECT dt1.*, rownum AS  rn
                FROM (
                    SELECT abs(t.create_time - ct.D) as diffTime, ct.D as cDate
                    FROM table(t1.CREATE_TIMES) ct
                    WHERE (t.EX_DIF <= 1 AND TO_DATE(t.create_time) = (TO_DATE(ct.D) + 1)) OR (t.EX_DIF > 1 AND TO_DATE(t.create_time) = TO_DATE(ct.D))
                    ORDER BY diffTime
                ) dt1
            )
            WHERE rn = 1) END) AS nearCT1
        FROM t_6 t
        LEFT JOIN t_8 t1
        ...
        SELECT t.*,
            ...
            FROM (
                ...
                FROM (
                    SELECT abs(t.create_time - ct.D) as diffTime, ct.D as cDate
                    FROM table(t1.CREATE_TIMES) ct
                    WHERE (t.EX_DIF <= 2 AND TO_DATE(t.create_time) = (TO_DATE(ct.D) + 2)) OR (t.EX_DIF > 2 AND TO_DATE(t.create_time) = TO_DATE(ct.D))
                    ORDER BY diffTime
                ) dt1
            )
            WHERE rn = 1) END) AS nearCT2
        FROM t_9 t
        LEFT JOIN t_8 t1
        ...
        SELECT t.*,
            ...
            FROM (
                ...
                FROM (
                    SELECT abs(t.create_time - ct.D) as diffTime, ct.D as cDate
                    FROM table(t1.CREATE_TIMES) ct
                    WHERE (t.EX_DIF <= 3 AND TO_DATE(t.create_time) = (TO_DATE(ct.D) + 3)) OR (t.EX_DIF > 3 AND TO_DATE(t.create_time) = TO_DATE(ct.D))
                    ORDER BY diffTime
                ) dt1
            )
            WHERE rn = 1) END) AS nearCT3
        FROM t_10 t
        LEFT JOIN t_8 t1
        ...
        SELECT t.*,
            ...
            FROM (
                ...
                FROM (
                    SELECT abs(t.create_time - ct.D) as diffTime, ct.D as cDate
                    FROM table(t1.CREATE_TIMES) ct
                    WHERE TO_DATE(t.create_time) = TO_DATE(ct.D)
                    ORDER BY diffTime
                ) dt1
            )
            WHERE rn = 1) END) AS nearCT4
        FROM t_11 t
        LEFT JOIN t_8 t1
        ON t.FLIGHT_DATE = t1.FLIGHT_DATE - 1 and t.DEP = t1.DEP AND t.arr = t1.arr
    ),
